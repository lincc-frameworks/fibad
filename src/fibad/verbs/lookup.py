...
class Lookup(Verb):
    ...
    def setup_parser(parser: ArgumentParser):
        ...
        """
        parser.add_argument("-i", "--id", type=str, help="ID of image", required=True)
    ...
    def run(self, id: str) -> Optional[np.ndarray]:
        ...
        results_dir = (
            self.config["results"]["inference_dir"]
            if self.config["results"]["inference_dir"]
            else find_most_recent_results_dir(self.config, verb="infer")
        )
        ...
        batch_index_path: Path = results_dir / "batch_index.npy"
